...
impl WorkTool {
    ...
    fn main_menu(&mut self) -> Result<MenuResult, Box<dyn std::error::Error>> {
        ...
        let selection = Select::new(&format!("[{}] Select option:", self.get_context_display()), options)
            .prompt()?;
    ...
    fn configure_menu(&mut self) -> Result<MenuResult, Box<dyn std::error::Error>> {
        ...
        let selection = Select::new(&format!("[{}] Select service:", self.get_context_display()), options)
            .prompt()?;
    ...
    fn service_menu(&mut self) -> Result<MenuResult, Box<dyn std::error::Error>> {
        ...
        let selection = Select::new(&format!("[{}] Configure {}:", self.get_context_display(), service), options)
            .prompt()?;
        ...
        match selection {
            "Set URL" => {
                ...
                self.config.entry(service.clone())
                    .or_default()
            ...
            "Set Organization/Project" => {
                let org = Text::new("Enter organization/project:")
                    .prompt()?;
    ...
    fn filter_menu(&mut self) -> Result<MenuResult, Box<dyn std::error::Error>> {
        ...
        let selection = Select::new(&format!("[{}] Filter Options:", self.get_context_display()), options)
            .prompt()?;
        ...
        match selection {
            "Set Project Filter" => {
                ...
                if project != "All Projects" {
                    self.filters.insert("project".to_string(), project.to_string());
            ...
            "Set Person Filter" => {
                let person = Text::new("Enter person name or email:")
                    .prompt()?;
            ...
            "Set Status Filter" => {
                ...
                let status_options = MultiSelect::new("Select statuses (space to select):", statuses)
                    .prompt()?;
                
                if !status_options.is_empty() && !status_options.contains(&"All Statuses") {
                    self.filters.insert("status".to_string(), status_options.join(", "));
            ...
            "Set Date Range" => {
                let ranges = vec!["Last 7 days", "Last 30 days", "Last 90 days", "Custom", "No filter"];
                ...
                match range {
                    "Custom" => {
                        ...
                        self.filters.insert("date_range".to_string(), format!("{} to {}", start, end));
                    },
                    "No filter" => {
                        self.filters.remove("date_range");
                    },
                    _ => {
                        self.filters.insert("date_range".to_string(), range.to_string());
                    }
                }
    ...
    fn view_menu(&mut self) -> Result<MenuResult, Box<dyn std::error::Error>> {
        ...
        let selection = Select::new(&format!("[{}] View Data:", self.get_context_display()), options)
            .prompt()?;
        ...
        match selection {
            "Dashboard View" | "Table View" | "Card View" | "By Project" | "By Person" => {
                ...
                self.show_filters();
                println!("(This would launch ratatui with {} layout)", selection.to_lowercase());

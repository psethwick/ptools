...
(* Simple JSON5-like preprocessing - strips comments and handles trailing commas *)
let preprocess_json5 content =
  ...
  let processed_lines =
    List.map
      (fun line ->
        ...
        | Some i when i < String.length line - 1 && line.[i + 1] = '/' ->
  ...
  let rec remove_block_comments acc chars =
    match chars with
    | '/' :: '*' :: rest -> skip_block_comment rest
    | c :: rest -> remove_block_comments (c :: acc) rest
    | [] -> List.rev acc
  and skip_block_comment chars =
    match chars with
    | '*' :: '/' :: rest -> remove_block_comments acc rest
    | _ :: rest -> skip_block_comment rest
    | [] -> List.rev acc (* Unclosed comment, return what we have *)
  in

  let chars = String.to_seq content |> List.of_seq in
  let filtered_chars = remove_block_comments [] chars in
  let content = String.of_seq (List.to_seq filtered_chars) in

  (* Remove trailing commas (simple regex-like approach) *)
  let content =
    Str.global_replace (Str.regexp ",[ \t\n]*\\([]}]\\)") "\\1" content
  in
  content

(* File reading with JSON5-like preprocessing *)
let read_tasks_from_file path =
  try
    let content = In_channel.with_open_text path In_channel.input_all in
    let preprocessed = preprocess_json5 content in
    let json = Yojson.Safe.from_string preprocessed in
    Ok (tasks_file_of_json json)
  with
  | Sys_error msg -> Error ("Failed to read tasks.json: " ^ msg)
  | Yojson.Json_error msg ->
      Error ("Failed to parse JSON after preprocessing: " ^ msg)
